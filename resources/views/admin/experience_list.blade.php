                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach($list as $item)
                                <tr id="{{$item->id}}">
                                    <td>{{$item->id}}</td>
                                    <td><a href="{{route('admin.experience.add', ['experienceID' => $item->id])}}" class="btn btn-warning editExperience">Düzenle<i class="mdi mdi-pencil-box-outline"></i></a></td>
                                    <td><a data-id="{{$item->id}}" href="javascript:void(0)" class="btn btn-danger deleteExperience">Sil<i class="mdi mdi-delete-forever"></i></a></td>
                                    <td>{{$item->order}}</td>
                                    <td>{{$item->experience_date}}</td>
                                    <td>{{$item->experience_info}}</td>
           ...
           $.ajax({
               ...
               success: function (response){
                   Swal.fire({
                       icon: 'success',
                       title: 'Başarılı!',
                       text: response.experienceID + " ID'li kayıt durumu " + response.newStatus + " olarak güncellenmeiştir.",
