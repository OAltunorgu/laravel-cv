                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach($list as $item)
                                <tr id="{{$item->id}}">
                                    <td>{{$item->id}}</td>
                                    <td><a href="{{route('admin.education.add', ['educationID' => $item->id])}}" class="btn btn-warning editEducation">Düzenle<i class="mdi mdi-pencil-box-outline"></i></a></td>
                                    <td><a data-id="{{$item->id}}" href="javascript:void(0)" class="btn btn-danger deleteEducation">Sil<i class="mdi mdi-delete-forever"></i></a></td>
                                    <td>{{$item->order}}</td>
                                    <td>{{$item->education_date}}</td>
                                    <td>{{$item->education_info}}</td>
           ...
           $.ajax({
               ...
               success: function (response){
                   Swal.fire({
                       icon: 'success',
                       title: 'Başarılı!',
                       text: response.educationID + " ID'li kayıt durumu " + response.newStatus + " olarak güncellenmeiştir.",
